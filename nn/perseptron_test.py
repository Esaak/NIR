...
class DataLoader:
    ...
    def load_from_folders(base_path: str, feature_filename: str, target_filename: str, 
        ...
        folder_paths = [os.path.join(base_path, "output_19_01_2025_2"), os.path.join(base_path, "output_21_03_2025"),
                        os.path.join(base_path, "output_31_03_2025")]
        X = pd.DataFrame()
        y = pd.DataFrame()
...
class ModelTrainer:
    def __init__(
        ...
        self.criterion = criterion if criterion else MSELoss_custom(reduction='none')
        # self.criterion_1 = criterion if criterion else MSELoss_custom()
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            # for idx, batch in enumerate(dataloader):
                ...
                # batch_weights= batch_weights/torch.sum(batch_weights).item()
                ...
                # if idx % 3 == 0:
                #     tb_writer.add_scalar('train_batch_loss_none', epoch_loss/(idx+1), global_step = epoch*(int(len(dataloader)/batch_size))+idx + 1)
                #     tb_writer.add_scalar('train_batch_loss_mean', epoch_loss_1/(idx+1), global_step = epoch*(int(len(dataloader)/batch_size))+idx + 1)
                #     tb_writer.add_scalar('delta_losses', (epoch_loss - epoch_loss_1)/(idx+1), global_step = epoch*(int(len(dataloader)/batch_size))+idx + 1)
    ...
    def evaluate(self, X_test: np.ndarray, y_test: pd.DataFrame) -> Tuple[pd.DataFrame, float]:
        ...
        with torch.no_grad():
            ...
            mse_loss = self.criterion(test_outputs, y_test_tensor).mean().item()
...
def plot_feature_importance(models, X_test, target_names, feature_names, save_path):
    ...
    for target in target_names:
        ...
        background_tensor = torch.FloatTensor(background).to(models[target].model[0].weight.device)
        
        # Define a prediction function for SHAP
        def predict_fn(x):
            x_tensor = torch.FloatTensor(x).to(models[target].model[0].weight.device)
            with torch.no_grad():
                return models[target](x_tensor).cpu().numpy()
        
        # Initialize the SHAP explainer
        explainer = shap.DeepExplainer(models[target], background_tensor)
        
        # Calculate SHAP values
        X_test_sample = X_test.sample(min(1000, len(X_test)), random_state=42).to_numpy()
        shap_values = explainer.shap_values(torch.FloatTensor(X_test_sample).to(models[target].model[0].weight.device))
        shap_values_d[target] = shap_values
    ...
    for idx, col in enumerate(target_names):
        ...
        shap_values_tmp = shap_values_d[col][:, :]
        shap_values_tmp_abs = np.abs(shap_values_tmp)

        combined_shap_values = np.mean(shap_values_tmp_abs, axis=0)
        feature_importance = pd.DataFrame({
            "Feature": np.squeeze(feature_names),
            "Importance": np.squeeze(combined_shap_values)
            })
        feature_importance_pd = pd.Series(np.squeeze(combined_shap_values), np.squeeze(feature_names))
...
def main():
    ...
    config = {
        ...
        'output_dir': './outputs_02_04_2025',
        'model_checkpoints_dir': './model_checkpoints_02_04_2025'
    }
    tb_writer = SummaryWriter(log_dir='./outputs_02_04_2025/logs/run004/')
    ...
    plot_eda(pd.DataFrame(data["y_test"]), config['output_dir'] + "/target_test.png")
    plot_eda(pd.DataFrame(data["y_train"]), config['output_dir'] + "/target_train_transform.png")


    y_pred = {}
    models = {}
    for target in data["y_train"].columns:
        ...
        y_pred_tmp, mse_loss = trainer.evaluate(data['X_test'], pd.DataFrame(data=np.array(data['y_test'][target]), columns=[target]))
        y_pred[target] = np.squeeze(np.array(y_pred_tmp))
    ...
    logger.info(f"Final Test MSE Loss: {mse_loss:.4f}")
